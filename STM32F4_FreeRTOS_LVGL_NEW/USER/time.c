...
__align(4) uint8_t rtc_time[40]  = {0};
__align(4) uint8_t rtc_date[40]  = {0};
__align(4) uint8_t rtc_start[40] = {0};
...
uint8_t My_RTC_Init(void)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);//使能PWR时钟
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(19,9,56,RTC_H12_AM);	//设置时间
		RTC_Set_Date(24,3,24,6);		//设置日期
...
void My_RTC_Get(void) //获取时间备用
{
	...
	sprintf((char*)rtc_time,"Time:%02d:%02d:%02d",RTC_TimeStruct.RTC_Hours,RTC_TimeStruct.RTC_Minutes,RTC_TimeStruct.RTC_Seconds);
	RTC_GetDate(RTC_Format_BIN, &RTC_DateStruct);
	sprintf((char*)rtc_date,"Date:20%02d-%02d-%02d",RTC_DateStruct.RTC_Year,RTC_DateStruct.RTC_Month,RTC_DateStruct.RTC_Date);
}
